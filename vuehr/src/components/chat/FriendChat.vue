        ...
        <div class="chatDiv" ref="chatDiv" id="chatDiv">
          ...
          <template v-for="msg in msgList">
            ...
            <div style="display: flex;justify-content: flex-start;align-items: center;box-sizing: border-box;"
              v-if="msg.from==currentFriend.username" :key="msg.id">
            ...
            <!--发出去的消息-->
            <div v-if="msg.from!=currentFriend.username" style="display: flex;justify-content: flex-end;align-items: center;box-sizing: border-box;" :key="msg.id">
              <div
...
<script>
  export default{
    ...
    methods: {
      sendMsg(){
        var oldMsg = window.localStorage.getItem(this.$store.state.userInfo.username + "#" + this.currentFriend.username);
        if (oldMsg == null) {
          oldMsg = [];
          oldMsg.push({msg: this.msg, from: this.$store.state.userInfo.username});
          window.localStorage.setItem(this.$store.state.userInfo.username + "#" + this.currentFriend.username, JSON.stringify(oldMsg))
        } else {
          var oldMsgJson = JSON.parse(oldMsg);
          oldMsgJson.push({msg: this.msg, from: this.$store.state.userInfo.username});
          window.localStorage.setItem(this.$store.state.userInfo.username + "#" + this.currentFriend.username, JSON.stringify(oldMsgJson))
        }
        this.$store.state.ws.send( this.msg + ";" + this.$store.state.userInfo.username+";"+this.currentFriend.id);
        this.msg = '';
        this.updateChatDiv();
      },
      updateChatDiv(){
      ...
      loadHrs(){
        var _this = this;
        this.getRequest("/chat/getAllHrUsers").then(resp=> {
          _this.hrs = resp.data;
          _this.hrs.map(function(item,index){
            if(item.id==_this.$store.state.userInfo.id){
              _this.hrs.splice(index,1);
            }
          })
        })
      }

  ...
  export default {
    ...
    methods: {
      ...
      doDelete(id){
        ...
        this.deleteRequest("/deleteSalary/" + id).then(resp=>{
      ...
      next(){
         ...
         * trafficSalary: '',
          ...
          accumulationFundPer: ''
         */
        if(this.index==0&&!this.salary.basicSalary){
          this.$message({type: 'error', message: '字段不能为空!'});
          return;
        }else if(this.index==1&&!this.salary.trafficSalary){
          this.$message({type: 'error', message: '字段不能为空!'});
          return;
        }else if(this.index==2&&!this.salary.lunchSalary){
          this.$message({type: 'error', message: '字段不能为空!'});
          return;
        }else if(this.index==3&&!this.salary.bonus){
          this.$message({type: 'error', message: '字段不能为空!'});
          return;
        }else if(this.index==4&&!this.salary.pensionPer&&!this.salary.pensionBase){
          this.$message({type: 'error', message: '字段不能为空!'});
          return;
        }else if(this.index==5&&!this.salary.medicalBase&&!this.salary.medicalPer){
          this.$message({type: 'error', message: '字段不能为空!'});
          return;
        }else if(this.index==6&&!this.salary.accumulationFundBase&&!this.salary.accumulationFundPer){
          this.$message({type: 'error', message: '字段不能为空!'});
          return;
        }else if(this.index==7&&!this.salary.createDate){
          this.$message({type: 'error', message: '字段不能为空!'});
          return;
        }
      ...
      loadSalaryCfg(){
        ...
        var _this = this;
        this.getRequest("/salary/sob/getSlaryFDB").then(resp=> {
          _this.tableLoading = false;
          if (resp && resp.status == 200) {
           
            _this.salaries = resp.data.data;
          }
        })
      },
      emptySalary(){

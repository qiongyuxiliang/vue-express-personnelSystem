...
<script>
  export default{
    ...
    methods: {
      ...
      loadSalaries(){
        var _this = this;
        this.getRequest("/salary/getAccinformation").then(resp=> {
      ...
      updateSalaryCfg(eid){
        ...
        this.putRequest("/salary/updateAccinformation/", {eid: eid, sid: this.sid, osid: this.osid}).then(resp=> {
          if(resp&&resp.status==200) {
            var data = resp.data;
            _this.loadEmps();
          }
      ...
      loadEmps(){
        ...
        var _this = this;
        this.getRequest("/salary/getEmployeFDB?page=" + this.currentPage + "&size=10").then(resp=> {
          _this.tableLoading = false;
          if (resp && resp.status == 200) {
            let data = resp.data;
            
            _this.emps = data.data;
